...
export default function Header({
  title = "DietoCase",
  subtitle = "Laboratório interativo de Nutrição Clínica: Simulação clínica, anamnese interativa e prontuário virtual",
  activeModule = "home"
}: HeaderProps) {
  return (
    <header className="bg-slate-900 border-b border-slate-800 text-white px-4 sm:px-8 py-3.5 flex flex-wrap items-center justify-between gap-4 sticky top-0 z-30 shadow-md">
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-[11px] sm:text-xs text-slate-400 font-normal leading-tight max-w-xs sm:max-w-md md:max-w-xl mt-0.5">{subtitle}</p>
        </div>
      </div>
